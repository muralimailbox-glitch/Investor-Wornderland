...
// ── AI email composer ─────────────────────────────────────────────────────
// Lightweight inline composer scoped to this single investor. Calls
// /draft/compose with investorIds=[id] (server resolves the active lead),
// then sends via /draft/send. Keeps the founder in the activity drawer
// instead of bouncing them to bulk-email for a one-off message.
...
function AiEmailComposer({
  ...
  async function draft() {
    ...
    try {
      ...
      // the /draft/send route doesn't run the per-recipient template
      // substitutions that batch-create does.
